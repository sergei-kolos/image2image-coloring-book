...
def test_convert_returns_json_with_images():
    client = TestClient(app)
    r = client.post(
        "/api/convert",
        files={"image": ("t.png", make_two_color_bytes(), "image/png")},
        data={"detail_level": "5", "paper_size": "A4"},
    )
    assert r.status_code == 200
    ...
    assert body["colored"].startswith("data:image/png;base64,")
    assert body["outline"].startswith("data:image/png;base64,")
...
def test_index_page_served():
    ...
    r = client.get("/")
    assert r.status_code == 200
    assert "Paint by Numbers" in r.text
